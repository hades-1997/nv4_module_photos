...
if($id_row>0){
	$category_id=$db->query('SELECT category_id FROM '.TABLE_PHOTO_NAME.'_album where album_id='.$id_row)->fetchColumn();

	if(empty($category_id)){
		$category_id=0;
	}else{
		$parent_id=$db->query('SELECT parent_id FROM '.TABLE_PHOTO_NAME.'_category where category_id='.$category_id)->fetchColumn();
	}
}
//Xac dinh RSS
if( $module_info['rss'] )
